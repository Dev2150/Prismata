...
// ── compileShaderFile ─────────────────────────────────────────────────────────
// Compiles an HLSL source file into GPU bytecode (a "blob").
static ComPtr<ID3DBlob> compileShaderFile(const wchar_t *filename, const char *entry, const char *target) {
    ComPtr<ID3DBlob> blob;
    ComPtr<ID3DBlob> err;

    // 1. Print Current Working Directory (CWD) once for debugging
    static bool printedCWD = false;
    if (!printedCWD) {
        auto cwd = std::filesystem::current_path();
        std::string msg = "DEBUG: Current Working Directory: " + cwd.string() + "\n";
    ...
    // If input is "Shaders/Creature.hlsl"
    candidates.push_back(name);                         // Try: cmake-build-release/Shaders/Creature.hlsl
    candidates.push_back(L"../" + name);                // Try: cmake-build-release/../Shaders/Creature.hlsl (CORRECT for your setup)
    candidates.push_back(L"../../" + name);             // Try: Up 2 levels

    // If input is just "Creature.hlsl" (fallback)
    std::wstring justName = std::filesystem::path(filename).filename().wstring();
    candidates.push_back(L"Shaders/" + justName);       // Try: Shaders/Creature.hlsl
    candidates.push_back(L"../Shaders/" + justName);    // Try: ../Shaders/Creature.hlsl
    candidates.push_back(L"../../Shaders/" + justName); // Try: ../../Shaders/Creature.hlsl
    ...
    if (finalPath.empty()) {
        ...
        msg += "Checked the following paths:\n";
        for(const auto& p : candidates) {
...
bool Renderer::init(ID3D11Device *dev, ID3D11DeviceContext *c, int w, int h) {
    ...
    rd.FillMode = D3D11_FILL_SOLID;
    rd.CullMode = D3D11_CULL_NONE; // both sides — for FOV cone
...
bool Renderer::createShaders() {
    ...
    auto cvs = compileShaderFile(L"Creature.hlsl", "VSMain", "vs_5_0");
    auto cps = compileShaderFile(L"Creature.hlsl", "PSMain", "ps_5_0");
    ...
    auto svs = compileShaderFile(L"Simple.hlsl", "VSMain",  "vs_5_0");
    auto fps = compileShaderFile(L"Simple.hlsl", "FovPS",   "ps_5_0");
